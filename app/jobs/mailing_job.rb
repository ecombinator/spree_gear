class MailingJob < ApplicationJob
  ...
  def perform(*args)
    ...
    recipients += Spree::MailingRecipient.active_non_users if @mailing.deliver_to_non_users
    recipients += csv_recipients if @mailing&.csv&.path
    @mailing.send_to! recipients.reject(&:nil?).uniq
  end
  ...
  def user_recipients
    Spree::User.mailing_recipients.map do |user|
